...
export class BookingService {
    static async createBooking(data: CreateBookingDTO): Promise<BookingResponse> {
        const { room_id, start_time, end_time, user_id } = data;
        ...
        `;
            ...
            VALUES ($1, $2, $3, $4) 
            RETURNING id, room_id, start_time, end_time, user_id, created_at
        `;
            const result = await client.query(insertQuery, [room_id, start_time, end_time, user_id]);
    ...
    static async getBookings(userId: number): Promise<BookingResponse[]> {
        const result = await db.query(
            'SELECT id, room_id, start_time, end_time, user_id, created_at FROM bookings WHERE user_id = $1',
            [userId]
        );
        return result.rows;
    }
    ...
    static async getBooking(id: number, userId: number): Promise<BookingResponse> {
        const result = await db.query(
            'SELECT id, room_id, start_time, end_time, user_id, created_at FROM bookings WHERE id = $1 AND user_id = $2',
            [id, userId]
        );
    ...
    static async updateBooking(id: number, userId: number, data: CreateBookingDTO): Promise<BookingResponse> {
        ...
        try {
            ...
            const result = await client.query(
                `UPDATE bookings SET room_id = $1, start_time = $2, end_time = $3 
             WHERE id = $4 AND user_id = $5 
             RETURNING *`,
                [room_id, start_time, end_time, id, userId]
            );
    ...
    static async deleteBooking(id: number, userId: number): Promise<BookingResponse> {
        const result = await db.query(
            'DELETE FROM bookings WHERE id = $1 AND user_id = $2 RETURNING id, room_id, start_time, end_time, user_id, created_at',
            [id, userId]
        );
        if (result.rows.length === 0) {
